...
static	void	sysinit()
{
	...
	for (i = 0; i < NPROC; i++) {
		...
		prptr->prprio = 0;
		prptr->free_flag = 1;//1 stands for freeing the stack memory and 2 for not freeing the stack memory
	}
	...
	currpid = NULLPROC;
	prptr->free_flag = 1;//1 stands for freeing the stack memory and 2 for not freeing the stack memory
	//mypid = -99;//initializing with a negative value as the pid is never negative
