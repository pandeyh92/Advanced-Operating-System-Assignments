	...
	)
{
	...
	uint32		*saddr,*temp,size;		/* stack address		*/
	...
	//code addition for 9.4
	prptr = &proctab[currpid];
	if(prptr->free_flag == 2)
	{
		//kprintf("\nInside Create For 9.4--> custom made stack called for process id %d:\n",currpid);
		kprintf("\ncustom made stack called for process id %d:\n",currpid);
		if (((saddr = (uint32 *)getstk_new(ssize)) == (uint32 *)SYSERR ) || (pid=newpid()) == SYSERR || priority < 1 ) 
			{
			restore(mask);
			return SYSERR;
			}
		...
		{
		  if (((saddr = (uint32 *)getstk(ssize)) ==   (uint32 *)SYSERR ) || (pid=newpid()) == SYSERR || priority < 1 )
			 {
	                  restore(mask);
			  return SYSERR;
			 }
	
		}//code addition ends for 9.4

	/////code addition for 9.7
	//kprintf("\n----Inside Create for 9.7----\n");
	//kprintf("\n----Current Pid----%d\n",getpid());
	temp=saddr;
	size=ssize;
	//kprintf("\nStack base address: %u, size %u\n",saddr,size);
	while(size>0)
	{
	*temp=-99;
	temp--;
