...
syscall	future_get(future *f,int *value)
{
	...
	case FUTURE_SHARED : {
				if(f->state == FUTURE_EMPTY)
						{
							if(f->get_queue.count == NMAX)
								{
									kprintf("\n SET QUEUE IS FULL , CAN'T ENQUEUE MORE PROCESS");
									restore(mask);
									return SYSERR;
								}
							f->get_queue.head = (f->get_queue.head) % NMAX;
							f->get_queue.futqueue[f->get_queue.head++] = currpid;
							f->get_queue.count++;
							f->state = FUTURE_WAITING;
						  //	f->pid = currpid;
							cpid = &proctab[currpid];
							...
							if(f->set_queue.count > 0)
								{
									...
									cpid->prstate = PR_WAIT;
									f->set_queue.count--;
									ready(f->set_queue.futqueue[f->set_queue.tail++]);
