...
//struct  arptime   arptimecache[ARP_SIZ];/* Timer*/
...
void	arp_init(void)
{
	...
	for (i=1; i<ARP_SIZ; i++) {	/* Initialize cache to empty	*/
		...
		//arptimecache[i].arstate = AR_FREE;
	}
}
	...
	)
{
	...
	bool8	found;			/* Is the sender's address in	*/
					/*   the cache?			*/
        //struct arptime *artimeptr;	/* Ptr to ARP time cache entry	*/
	...
	found = FALSE;
       /*Added for 17.3*/
		...
		arptr = &arpcache[slot];
                  //kprintf("\tAlot: %d",slot);
               if(arpcache[slot].whenTime>0)
              {
		if ((clktime-arpcache[slot].whenTime) >300) 
                {
	...
	if (found) {
		...
		memcpy(arptr->arhaddr, pktptr->arp_sndha, ARP_HALEN);
                /*Added for assignment 7*/
                 ...
                 kprintf("\nf:t %d",clktime);
                 //arptr->whenTime=whenTime;
		//memcpy(arptr->arhaddr, pktptr->arp_sndha, ARP_HALEN);
		  arptr->whenTime=clktime;
		  //artimeptr->arpid=arptr->arpid; 	
                 /*End*/
		/*Added for assignment 7ends*/
