				...
				if(f->state == FUTURE_EMPTY)
						...
						{	
							...
							//while(f->get_queue.count != 0)
							{
								...
								//ipid = f->get_queue.fpid[f->get_queue.tail++];

								//kprintf("\n S:L %d ",f->get_queue.count ); 
								//f->get_queue.count --;	
								ready(f->get_queue.fpid[f->get_queue.tail++]);	
	...
	case FUTURE_QUEUE: {
				if(f->state == FUTURE_EMPTY || f->state == FUTURE_VALID)
						{	
							...
							f->set_queue.head = (f->set_queue.head) % NFQ;

							f->set_queue.fpid[f->set_queue.head++]=currpid;
							f->set_queue.count++;
							ptr = &proctab[currpid];
							ptr->prstate = PR_WAIT;
